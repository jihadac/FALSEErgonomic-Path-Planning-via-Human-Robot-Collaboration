...
class Person {
  constructor(initObj={}) {
    if (initObj === null) {
      ...
      if (initObj.hasOwnProperty('faceParts')) {
        this.faceParts = initObj.faceParts
      }
      else {
        this.faceParts = [];
      }
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    // Serialize message field [faceParts]
    // Serialize the length for message field [faceParts]
    bufferOffset = _serializer.uint32(obj.faceParts.length, buffer, bufferOffset);
    obj.faceParts.forEach((val) => {
      bufferOffset = BodyPart.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    // Deserialize message field [faceParts]
    // Deserialize array length for message field [faceParts]
    len = _deserializer.uint32(buffer, bufferOffset);
    data.faceParts = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.faceParts[i] = BodyPart.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    length += 24 * object.faceParts.length;
    return length + 16;
  }
  ...
  static Resolve(msg) {
    ...
    if (msg.faceParts !== undefined) {
      resolved.faceParts = new Array(msg.faceParts.length);
      for (let i = 0; i < resolved.faceParts.length; ++i) {
        resolved.faceParts[i] = BodyPart.Resolve(msg.faceParts[i]);
      }
    }
    else {
      resolved.faceParts = []
