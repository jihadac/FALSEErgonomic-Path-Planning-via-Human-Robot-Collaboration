...
void pick(moveit::planning_interface::MoveGroupInterface& move_group)
{
  ...
  Eigen::Quaterniond q(current_orientation.w, current_orientation.x, current_orientation.y, current_orientation.z);

  // Create rotation quaternion for 90 degree rotation around z-axis
  Eigen::Quaterniond q_rot(Eigen::AngleAxisd(-M_PI/2.0, Eigen::Vector3d::UnitZ()));

  // Combine quaternions to get new orientation quaternion
  q = q * q_rot;

  // Normalize quaternion to ensure it represents a valid orientation
  q.normalize();

  // Set new orientation quaternion for grasp pose
  grasps[0].grasp_pose.pose.orientation.x = q.x();
  grasps[0].grasp_pose.pose.orientation.y = q.y();
  grasps[0].grasp_pose.pose.orientation.z = q.z();
  grasps[0].grasp_pose.pose.orientation.w = q.w();

  grasps[0].grasp_pose.pose.position.x = 0;
  grasps[0].grasp_pose.pose.position.y = 0.500;
  grasps[0].grasp_pose.pose.position.z = 0.133;
