...
#include <franka_planner_control/erg_static_move_controller.h>
...
void ErgStaticMoveController::skeletonTrackingCallback(const zed_interfaces::ObjectsStamped::ConstPtr& msg){
  rviz_visualization.addAllSkeletons(msg -> objects);
}

void ErgStaticMoveController::robotTrajectoryCallback(const moveit_msgs::RobotTrajectory& msg){
  size_trajectory_ = msg.joint_trajectory.points.size();
  robot_trajectory_ = msg;
}

void ErgStaticMoveController::plannerReferenceCallback(const geometry_msgs::Pose& msg){
  xe_r_ = msg;
}

bool ErgStaticMoveController::init(hardware_interface::RobotHW* robot_hw,
                                           ros::NodeHandle& node_handle) {
  std::string arm_id;
  if (!node_handle.getParam("arm_id", arm_id)) {
    ROS_ERROR("ErgStaticMoveController: Could not read parameter arm_id");
    return false;
  }
  ...
  if (!node_handle.getParam("joint_names", joint_names) || joint_names.size() != 7) {
    ROS_ERROR(
        "ErgStaticMoveController: Invalid or no joint_names parameters provided, aborting "
        "controller init!");
    return false;
  }
  ...
  if (!node_handle.getParam("Kp", Kp_) || Kp_.size() != 7) {
    ROS_ERROR(
        "ErgStaticMoveController:  Invalid or no k_gain parameters provided, aborting "
        "controller init!");
    return false;
  }
  ...
  if (!node_handle.getParam("Kd", Kd_) || Kd_.size() != 7) {
    ROS_ERROR(
        "ErgStaticMoveController:  Invalid or no d_gain parameters provided, aborting "
        "controller init!");
    return false;
  }
  ...
  if (!node_handle.getParam("publish_rate", publish_rate)) {
    ROS_INFO_STREAM("ErgStaticMoveController: publish_rate not found. Defaulting to "
                    << publish_rate);
  }
  ...
  if (model_interface == nullptr) {
    ROS_ERROR_STREAM(
        "ErgStaticMoveController: Error getting model interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "ErgStaticMoveController: Exception getting model handle from interface: "
        << ex.what());
    return false;
  } 
  ...
  if (effort_joint_interface == nullptr) {
    ROS_ERROR_STREAM(
        "ErgStaticMoveController: Error getting effort joint interface from hardware");
    return false;
  }
  for (size_t i = 0; i < 7; ++i) {
    try {
      ...
      ROS_ERROR_STREAM(
          "ErgStaticMoveController: Exception getting joint handles: " << ex.what());
      return false;
    }
  }
  ...
  if (state_interface == nullptr) {
    ROS_ERROR("ErgStaticMoveController: Could not get state interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "ErgStaticMoveController: Exception getting state handle: " << e.what());
    return false;
  }
  ...
  /* ROS subscribers */
  robot_trajectory_sub_ = node_handle.subscribe("/Panda/RobotTrajectory",10,&ErgStaticMoveController::robotTrajectoryCallback,this);
  planner_reference_sub_ = node_handle.subscribe("/Panda/PlannerReference",10,&ErgStaticMoveController::plannerReferenceCallback,this);
  ...
  skeleton_objects_sub_ = node_handle.subscribe("/zed2/zed_node/obj_det/objects", 10, &ErgStaticMoveController::skeletonTrackingCallback, this);
  rviz_visualization.changeSkeletonsMarkersColor(0.9, 0.65, 0.5, 0.7);

  /* obstacles used in NF, DSM, and visualization */
  number_of_walls_ = 1; // try with 4
...
void ErgStaticMoveController::starting(const ros::Time& /*time*/ ) {
...
void ErgStaticMoveController::update(const ros::Time& /*time*/,
  ...
  if(elapsed_time_.toSec()== 0.0){
    ...
    threadNF_ = std::thread(&ErgStaticMoveController::navigationField, this);
    threadDSM_ = std::thread(&ErgStaticMoveController::trajectoryBasedDSM, this);
...
PLUGINLIB_EXPORT_CLASS(franka_planner_control::ErgStaticMoveController,
