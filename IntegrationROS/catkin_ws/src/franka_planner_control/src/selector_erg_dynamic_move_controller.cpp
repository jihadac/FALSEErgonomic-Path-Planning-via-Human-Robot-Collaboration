...
#include <franka_planner_control/selector_erg_dynamic_move_controller.h>
...
void SelectorErgDynamicMoveController::skeletonTrackingCallback(const zed_interfaces::ObjectsStamped::ConstPtr& msg){
  rviz_visualization.addAllSkeletons(msg -> objects);
}

void SelectorErgDynamicMoveController::robotTrajectoryCallback(const moveit_msgs::RobotTrajectory& msg){
  size_trajectory_ = msg.joint_trajectory.points.size();
  robot_trajectory_ = msg;
}

void SelectorErgDynamicMoveController::plannerReferenceCallback(const geometry_msgs::Pose& msg){
  xe_r_ = msg;
}

void SelectorErgDynamicMoveController::rvizOrientationProblemCallback(const std_msgs::Int8& msg){ 
  rviz_orientation_problem_ = msg.data;
}

bool SelectorErgDynamicMoveController::init(hardware_interface::RobotHW* robot_hw,
                                           ros::NodeHandle& node_handle) {
  std::string arm_id;
  if (!node_handle.getParam("arm_id", arm_id)) {
    ROS_ERROR("SelectorErgDynamicMoveController: Could not read parameter arm_id");
    return false;
  }
  ...
  if (!node_handle.getParam("joint_names", joint_names) || joint_names.size() != 7) {
    ROS_ERROR(
        "SelectorErgDynamicMoveController: Invalid or no joint_names parameters provided, aborting "
        "controller init!");
    return false;
  }
  ...
  if (!node_handle.getParam("Kp", Kp_) || Kp_.size() != 7) {
    ROS_ERROR(
        "SelectorErgDynamicMoveController:  Invalid or no k_gain parameters provided, aborting "
        "controller init!");
    return false;
  }
  ...
  if (!node_handle.getParam("Kd", Kd_) || Kd_.size() != 7) {
    ROS_ERROR(
        "SelectorErgDynamicMoveController:  Invalid or no d_gain parameters provided, aborting "
        "controller init!");
    return false;
  }
  ...
  if (!node_handle.getParam("publish_rate", publish_rate)) {
    ROS_INFO_STREAM("SelectorErgDynamicMoveController: publish_rate not found. Defaulting to "
                    << publish_rate);
  }
  ...
  if (model_interface == nullptr) {
    ROS_ERROR_STREAM(
        "SelectorErgDynamicMoveController: Error getting model interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "SelectorErgDynamicMoveController: Exception getting model handle from interface: "
        << ex.what());
    return false;
  } 
  ...
  if (effort_joint_interface == nullptr) {
    ROS_ERROR_STREAM(
        "SelectorErgDynamicMoveController: Error getting effort joint interface from hardware");
    return false;
  }
  for (size_t i = 0; i < 7; ++i) {
    try {
      ...
      ROS_ERROR_STREAM(
          "SelectorErgDynamicMoveController: Exception getting joint handles: " << ex.what());
      return false;
    }
  }
  ...
  if (state_interface == nullptr) {
    ROS_ERROR("SelectorErgDynamicMoveController: Could not get state interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "SelectorErgDynamicMoveController: Exception getting state handle: " << e.what());
    return false;
  }
  ...
  /* ROS subscribers */
  robot_trajectory_sub_ = node_handle.subscribe("/Panda/RobotTrajectory",1,&SelectorErgDynamicMoveController::robotTrajectoryCallback,this);
  planner_reference_sub_ = node_handle.subscribe("/Panda/PlannerReference",1,&SelectorErgDynamicMoveController::plannerReferenceCallback,this);
  rviz_orientation_problem_sub_ = node_handle.subscribe("/Panda/RvizOrientationProblem",1,&SelectorErgDynamicMoveController::rvizOrientationProblemCallback,this);
  ...
  skeleton_objects_sub_ = node_handle.subscribe("/zed2/zed_node/obj_det/objects", 1, &SelectorErgDynamicMoveController::skeletonTrackingCallback, this);
  ...
  /* define cylindrical obstacles */
  {
    ...
    // Eigen::Matrix<double, 3, 1> obst_cylinder1_startpoint = {0.420715, -0.221358, 0.0}; // defined in robot base frame
    // Eigen::Matrix<double, 3, 1> obst_cylinder1_endpoint = {0.420715, -0.221358, 0.21}; // defined in robot base frame
    ...
    // Eigen::Matrix<double, 3, 1> obst_cylinder2_startpoint = {0.34745, 0.239329, 0.0}; // defined in robot base frame
    // Eigen::Matrix<double, 3, 1> obst_cylinder2_endpoint = {0.34745, 0.239329, 0.28}; // defined in robot base frame
    ...
    // for(int i=0; i <number_of_moveit_cylinders_; i++){ // to check if it overlaps with the MoveIt obstacle
  ...
  start_receivedtraj_index = 4; //3; 

  return true;
}

void SelectorErgDynamicMoveController::starting(const ros::Time& /*time*/ ) {
...
void SelectorErgDynamicMoveController::update(const ros::Time& /*time*/,
  ...
  if(elapsed_time_.toSec()== 0.0){
    ...
    q_r_ = referenceSelector(start_receivedtraj_index); // updates also q_r in ERG class
    ...
    threadNF_ = std::thread(&SelectorErgDynamicMoveController::navigationField, this);
    threadDSM_ = std::thread(&SelectorErgDynamicMoveController::trajectoryBasedDSM, this);
  ...
  if(elapsed_time_trajpred_.toSec()>= 0.01 /*pred_interval_*/){
    ...
    /* visualize end-effector reference pose */
    rviz_visualization.addEndEffectorReferenceFrame(xe_r_,rviz_orientation_problem_,"end-effector reference pose"); 
...
PLUGINLIB_EXPORT_CLASS(franka_planner_control::SelectorErgDynamicMoveController,
