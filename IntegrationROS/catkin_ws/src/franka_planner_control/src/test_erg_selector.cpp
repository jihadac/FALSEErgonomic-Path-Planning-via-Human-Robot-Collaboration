...
#include <franka_planner_control/test_erg_selector.h>
...
void TestErgSelector::skeletonTrackingCallback(const zed_interfaces::ObjectsStamped::ConstPtr& msg){
  rviz_visualization.addAllSkeletons(msg -> objects);
}

void TestErgSelector::robotTrajectoryCallback(const moveit_msgs::RobotTrajectory& msg){
  size_trajectory_ = msg.joint_trajectory.points.size();
  robot_trajectory_ = msg;
}

void TestErgSelector::plannerReferenceCallback(const geometry_msgs::Pose& msg){
  xe_r_ = msg;
}

bool TestErgSelector::init(hardware_interface::RobotHW* robot_hw,
                                           ros::NodeHandle& node_handle) {
  std::string arm_id;
  if (!node_handle.getParam("arm_id", arm_id)) {
    ROS_ERROR("TestErgSelector: Could not read parameter arm_id");
    return false;
  }
  ...
  if (!node_handle.getParam("joint_names", joint_names) || joint_names.size() != 7) {
    ROS_ERROR(
        "TestErgSelector: Invalid or no joint_names parameters provided, aborting "
        "controller init!");
    return false;
  }
  ...
  if (!node_handle.getParam("Kp", Kp_) || Kp_.size() != 7) {
    ROS_ERROR(
        "TestErgSelector:  Invalid or no k_gain parameters provided, aborting "
        "controller init!");
    return false;
  }
  ...
  if (!node_handle.getParam("Kd", Kd_) || Kd_.size() != 7) {
    ROS_ERROR(
        "TestErgSelector:  Invalid or no d_gain parameters provided, aborting "
        "controller init!");
    return false;
  }
  ...
  if (!node_handle.getParam("publish_rate", publish_rate)) {
    ROS_INFO_STREAM("TestErgSelector: publish_rate not found. Defaulting to "
                    << publish_rate);
  }
  ...
  if (model_interface == nullptr) {
    ROS_ERROR_STREAM(
        "TestErgSelector: Error getting model interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "TestErgSelector: Exception getting model handle from interface: "
        << ex.what());
    return false;
  } 
  ...
  if (effort_joint_interface == nullptr) {
    ROS_ERROR_STREAM(
        "TestErgSelector: Error getting effort joint interface from hardware");
    return false;
  }
  for (size_t i = 0; i < 7; ++i) {
    try {
      ...
      ROS_ERROR_STREAM(
          "TestErgSelector: Exception getting joint handles: " << ex.what());
      return false;
    }
  }
  ...
  if (state_interface == nullptr) {
    ROS_ERROR("TestErgSelector: Could not get state interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "TestErgSelector: Exception getting state handle: " << e.what());
    return false;
  }
  ...
  /* ROS subscribers */
  robot_trajectory_sub_ = node_handle.subscribe("/Panda/RobotTrajectory",10,&TestErgSelector::robotTrajectoryCallback,this);
  planner_reference_sub_ = node_handle.subscribe("/Panda/PlannerReference",10,&TestErgSelector::plannerReferenceCallback,this);
  ...
  skeleton_objects_sub_ = node_handle.subscribe("/zed2/zed_node/obj_det/objects", 10, &TestErgSelector::skeletonTrackingCallback, this);
...
void TestErgSelector::starting(const ros::Time& /*time*/ ) {
...
void TestErgSelector::update(const ros::Time& /*time*/,
  ...
  if(elapsed_time_.toSec()== 0.0){
    ...
    threadNF_ = std::thread(&TestErgSelector::navigationField, this);
    threadDSM_ = std::thread(&TestErgSelector::trajectoryBasedDSM, this);
...
PLUGINLIB_EXPORT_CLASS(franka_planner_control::TestErgSelector,
