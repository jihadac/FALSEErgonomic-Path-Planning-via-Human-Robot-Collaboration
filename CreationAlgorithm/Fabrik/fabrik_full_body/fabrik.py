import numpy as np
import math
from Fabrik.fabrik_full_body.constraints import Constraints
from Fabrik.fabrik_full_body.draw import Draw
from Fabrik.fabrik_full_body.angle_calculator.angle_calculator import AngleCalculator
import Fabrik.fabrik_full_body.util as util
from Fabrik.fabrik_full_body.input_reader import *
from Fabrik.fabrik_full_body.output_writer import *
...
class FABRIK:
    ...
    def laterality(self):
        ...
        if right_upper_body <= left_upper_body:
            return "right"
        if right_upper_body > left_upper_body: # was originally left, change to right such that we only consider right handed person
            return "right"
    ...
    def forward_arm(self, arm_index, target, target_orientation):
        ...
        for i in range(n - 2, -1, -1):
            ...
            if i < n - 2:
                ...
                if isinstance(constraint_return, bool):
                    if constraint_return == True:
                        error = True
                        break
    ...
    def drawing(self,counter,colors,a,b,c,step,height_circle,distance_circle):
        ...
        draw_obj.draw_final(counter,colors,a,b,c,step,height_circle,distance_circle)
