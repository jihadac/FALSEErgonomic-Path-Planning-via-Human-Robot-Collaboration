...
class Constraints:
    ...
    def rotational_constraint(self):
            ...
            if np.inner(np.cross(uv_i_before, unit_vec_i_o), (uv_normal_plane)) > 0:
                ...
                if self.theta[3] != 0:
                    ...
                    if angle <= self.theta[3]:
                        ...
                        p_nearest_point = p_i + int(l_before_i) * p_up
        ...
        if self.constraint_type == "BALL":
            ...
            inbound = 0
            if (q3 != 0 and q2 != 0) and round(((x_t ** 2) / (q3 ** 2) + (y_t ** 2) / (q2 ** 2))) <= 1 and sector == 1:
                inbound = 1
                p_prime = [0, 0, 0]
                return p_prime
            elif (q1 != 0 and q2 != 0) and round(((x_t ** 2) / (q1 ** 2) + (y_t ** 2) / (q2 ** 2))) <= 1 and sector == 2:
                inbound = 1
                p_prime = [0, 0, 0]
                return p_prime
            elif (q1 != 0 and q4 != 0) and round(((x_t ** 2) / (q1 ** 2) + (y_t ** 2) / (q4 ** 2))) <= 1 and sector == 3:
                inbound = 1
                p_prime = [0, 0, 0]
                return p_prime
            elif (q3 != 0 and q4 != 0) and round(((x_t ** 2) / (q3 ** 2) + (y_t ** 2) / (q4 ** 2))) <= 1 and sector == 4:
                inbound = 1
                p_prime = [0, 0, 0]
                return p_prime
            elif (q3 == 0 and q2 == 0) or (q1 == 0 and q2 == 0) or (q1 == 0 and q4 == 0) or (q3 == 0 and q4 == 0):
                error = True
                return error
